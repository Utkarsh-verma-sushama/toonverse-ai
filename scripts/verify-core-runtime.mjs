...
const worker=(await Promise.all(["worker.mjs","chat-billing.mjs","chat-execution.mjs"].map(name=>readFile(new URL("../backend/"+name,import.meta.url),"utf8")))).join("\n");
for(const guard of ["CHAT_EXECUTION_ENABLED","VERIFIED_IDENTITY_REQUIRED","PRICE_SNAPSHOT_REQUIRED","INSUFFICIENT_CREDITS","RATE_LIMIT_REACHED","DAILY_QUOTA_REACHED","MONTHLY_QUOTA_REACHED","GLOBAL_SPEND_CEILING_REACHED","releaseChat","settleChat"])if(!worker.includes(guard))throw new Error("Missing chat execution guard: "+guard);
...
for(const flag of ["chatCore","autonomousAgents","multimodalModelRouting","cloudBackend"])if(!config.includes(flag+": false"))throw new Error(flag+" must default to safe-off.");

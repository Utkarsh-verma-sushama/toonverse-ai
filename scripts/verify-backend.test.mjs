...
test('sensitive approval cannot authorize a different persisted agent step',async()=>{
 const db=database();try{
  ...
  db.sql.prepare("INSERT INTO agent_steps (id,run_id,sequence_no,tool_name,status,input_hash) VALUES ('step-share','run-alice',1,'share_project','awaiting_approval','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')").run();
  db.sql.prepare("UPDATE agent_approvals SET action_type='share_project',step_id='step-share',input_hash='aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',decision='approve',decided_at=? WHERE id='approval-alice'").run(now);
  ...
  await assert.rejects(authorizeAgentToolForRun(db,{runId:'run-alice',owner:'alice',toolName:'share_project',approvalId:'approval-alice',stepId:'step-delete',inputHash:'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'}),e=>e.code==='AGENT_TOOL_APPROVAL_STEP_MISMATCH');
  await assert.rejects(authorizeAgentToolForRun(db,{runId:'run-alice',owner:'alice',toolName:'share_project',approvalId:'approval-alice',stepId:'missing-step',inputHash:'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'}),e=>e.code==='AGENT_TOOL_APPROVAL_STEP_MISMATCH');
  await assert.rejects(authorizeAgentToolForRun(db,{runId:'run-alice',owner:'alice',toolName:'share_project',approvalId:'approval-alice',stepId:'step-share',inputHash:'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'}),e=>e.code==='AGENT_TOOL_APPROVAL_STEP_MISMATCH');
...
test('sensitive approval atomically claims exact step only once',async()=>{
 const db=database();try{
  ...
  await assert.rejects(authorizeAgentToolForRun(db,{runId:'run-alice',owner:'alice',toolName:'share_project',approvalId:'approval-alice',stepId:'step-share',inputHash:'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'}),e=>e.code==='AGENT_TOOL_STEP_MISMATCH'||e.code==='AGENT_TOOL_STEP_ALREADY_CLAIMED');
 }finally{db.sql.close();}
});

test('atomic step claim revalidates persisted approval state',async()=>{
 for(const mutation of [
  "UPDATE agent_approvals SET decision='deny' WHERE id='approval-alice'",
  ...
  const db=database();try{
   ...
   const step=db.sql.prepare("SELECT status,started_at AS startedAt FROM agent_steps WHERE id='step-share'").get();assert.equal(step.status,'awaiting_approval');assert.equal(step.startedAt,null);
  }finally{db.sql.close();}
 }
});

test('approved approval cannot be rebound to a mismatched action',()=>{
 const db=database();try{
  db.sql.prepare("INSERT INTO agent_steps (id,run_id,sequence_no,tool_name,status,input_hash) VALUES ('step-share','run-alice',1,'share_project','awaiting_approval','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')").run();
  db.sql.prepare("UPDATE agent_approvals SET action_type='share_project',step_id='step-share',input_hash='aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',decision='approve',decided_at=? WHERE id='approval-alice'").run(new Date().toISOString());
  assert.throws(()=>db.sql.prepare("UPDATE agent_approvals SET action_type='delete_project' WHERE id='approval-alice'").run(),/AGENT_APPROVAL_STEP_BINDING_REQUIRED/);
 }finally{db.sql.close();}
});
...
test('one approval cannot authorize a different same-action step',async()=>{
 const db=database();try{
  db.sql.prepare("INSERT INTO agent_steps (id,run_id,sequence_no,tool_name,status,input_hash) VALUES ('step-share','run-alice',1,'share_project','awaiting_approval','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')").run();
  db.sql.prepare("INSERT INTO agent_steps (id,run_id,sequence_no,tool_name,status,input_hash) VALUES ('step-share-two','run-alice',2,'share_project','awaiting_approval','bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')").run();
  db.sql.prepare("UPDATE agent_approvals SET action_type='share_project',step_id='step-share',input_hash='aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',decision='approve',decided_at=? WHERE id='approval-alice'").run(new Date().toISOString());
  await assert.rejects(authorizeAgentToolForRun(db,{runId:'run-alice',owner:'alice',toolName:'share_project',approvalId:'approval-alice',stepId:'step-share-two',inputHash:'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'}),e=>e.code==='AGENT_TOOL_APPROVAL_STEP_MISMATCH');
 }finally{db.sql.close();}
});

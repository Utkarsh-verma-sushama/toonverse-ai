...
test('staging migration chain is D1-compatible in Miniflare, not only SQLite',async()=>{
 ...
 try{
  ...
  try{for(const line of sql.split('\\n')){if(!line.trim()||line.trim().startsWith('--'))continue;pending+=line+'\\n';if(!line.trim().endsWith(';'))continue;try{parser.exec(pending);}catch(error){if(String(error.message).includes('incomplete input'))continue;throw error;}const complete=pending;if(!complete.trim().startsWith('PRAGMA'))queries.push(complete);pending='';}assert.equal(pending,'');}finally{parser.close();}

...
];let state=JSON.parse(localStorage.getItem("uvenaro-launch-readiness")||"{}");
function render(){const list=$("gates");list.innerHTML=checks.map(x=>'<article class="gate"><label><input type="checkbox" data-id="'+x[0]+'" '+(state[x[0]]?"checked":"")+'> <strong>'+x[1]+'</strong></label><p>'+x[3]+'</p><a href="'+x[2]+'" '+(x[2].startsWith("http")?'target="_blank" rel="noopener noreferrer"':"")+'">Open requirement</a></article>').join("");list.querySelectorAll("input").forEach(i=>i.onchange=()=>{state[i.dataset.id]=i.checked;localStorage.setItem("uvenaro-launch-readiness",JSON.stringify(state));summary()});summary()}

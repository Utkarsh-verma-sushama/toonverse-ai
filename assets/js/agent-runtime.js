(() => {
"use strict";
const DB="uvenaro-agent-runs",VERSION=1,STORE="runs";
...
async function request(path,options={}){
 ...
 const token=await window.UvenaroAuth?.getAccessToken?.().catch(()=>null);if(token)headers.set("Authorization",`Bearer ${token}`);
...
window.UvenaroAgents=Object.freeze({connected:()=>Boolean(base()),normalize,start,status,approve,cancel,isTerminal:status=>terminal.has(status),approvalActions:Object.freeze([...risky])});
})();
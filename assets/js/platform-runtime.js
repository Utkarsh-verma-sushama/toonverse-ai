...
function apply(){root.dataset.platform=profile.platform;root.dataset.nativePlatform=String(profile.native);if(profile.native&&!document.getElementById("toonverse-native-safe-area")){const style=document.createElement("style");style.id="toonverse-native-safe-area";style.textContent="html[data-native-platform=true] body{padding-top:max(env(safe-area-inset-top,0px),24px)!important}html[data-native-platform=true] .site-header{top:0}";document.head.append(style);}root.dataset.input=profile.tv?"remote":profile.coarse?"touch":"pointer";root.toggleAttribute("data-reduced-motion",profile.reducedMotion);root.toggleAttribute("data-save-data",profile.saveData);root.style.setProperty("--tv-safe-inline",profile.tv?"5vw":"0px");root.style.setProperty("--tv-safe-block",profile.tv?"4vh":"0px")}
